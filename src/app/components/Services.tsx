import { ClipboardCheck, Eye, Flame, HeartPulse, ShieldCheck, Stethoscope } from "lucide-react";
import { useState } from "react";

export function Services() {
  const [expandedService, setExpandedService] = useState<string | null>(null);

  const toggleProducts = (serviceTitle: string) => {
      ...
      <div className="container mx-auto px-6 lg:px-12 flex flex-col items-center">
        ...
        <div className="flex gap-6 mb-12 flex-wrap justify-center">
          <div className="bg-[#363B32] text-white shadow-lg rounded-xl w-48 h-48 flex flex-col justify-center items-center border border-[#4D5745] transform transition-all duration-300 hover:scale-105 hover:shadow-2xl mb-6 md:mb-0">
            ...
            <h3 className="text-lg font-semibold text-[#E6DCC5] mb-2 text-center">Consult with a Doctor</h3>
            <a
              href="/doctors"
          ...
          <div className="bg-[#363B32] text-white shadow-lg rounded-xl w-48 h-48 flex flex-col justify-center items-center border border-[#4D5745] transform transition-all duration-300 hover:scale-105 hover:shadow-2xl mb-6 md:mb-0">
            ...
            <h3 className="text-lg font-semibold text-[#E6DCC5] mb-2 text-center">Self-Assessment</h3>
            <a
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-10">
          {servicesData.map((service) => (
            <div key={service.title} className="relative bg-[#363B32] text-white shadow-lg rounded-2xl p-8 border border-[#4D5745] transform transition-all duration-300 hover:scale-105 hover:shadow-2xl">
              <div className="flex justify-center items-center mb-4 text-[#E91E63]">{service.icon}</div>
              <h3 className="text-xl sm:text-2xl font-semibold text-[#E6DCC5] mb-2">{service.title}</h3>
              <p className="text-[#E6DCC5] mb-6">{service.description}</p>
              <button
                onClick={() => toggleProducts(service.title)}
                ...
                <div className="mt-6">
                  <div className="flex overflow-x-auto space-x-6 p-4 pb-0">
                    {service.products.map((product) => (
                      <div key={product.name} className="bg-[#2A2D25] p-4 rounded-lg shadow-lg border border-[#4D5745] flex-shrink-0 w-64">
                        <div className="flex items-center gap-4">
                          ...
                          <div className="text-left">
                            ...
                            <div className="flex items-center gap-2 mt-3">
                              <button className="px-3 py-1 bg-gray-700 text-white rounded-lg">-</button>
...
const servicesData = [
  {
    ...
    products: [
      { name: "Delay Spray", description: "Fast-acting desensitizer", price: 19.99, rating: 4.5, image: "/images/delay-spray.png" },
      { name: "Herbal Tablets", description: "Natural endurance booster", price: 29.99, rating: 4.3, image: "/images/herbal-tablets.png" }
    ]
  },
  {
    ...
    products: [
      { name: "ED Tablets", description: "Clinically proven treatment", price: 49.99, rating: 4.8, image: "/images/ed-tablets.png" },
      { name: "Vacuum Device", description: "Non-invasive solution", price: 89.99, rating: 4.2, image: "/images/vacuum-device.png" }
    ]
  },
  {
    ...
    products: [
      { name: "Testosterone Booster", description: "Improve vitality & strength", price: 39.99, rating: 4.6, image: "/images/testosterone-booster.png" },
      { name: "Multivitamin", description: "Daily nutrient support", price: 19.99, rating: 4.4, image: "/images/multivitamin.png" }
    ]
  },
  {
    ...
    products: [
      { name: "Therapy Sessions", description: "One-on-one expert guidance", price: 99.99, rating: 4.9, image: "/images/therapy.png" },
      { name: "Self-Help Book", description: "Science-backed techniques", price: 24.99, rating: 4.7, image: "/images/self-help-book.png" }
    ]
  }
];
